...
public class StreamAssignment {
    ...
    public void task10(){
        ...
        optional = people.stream().filter(person -> person.getPersonId() == personId)
                .findFirst().map(person -> person.getDateOfBirth().getDayOfWeek() + " " + person.getDateOfBirth().getDayOfMonth()+ " " +
    ...
    public void task11(){
        ...
        averageAge = (double) people.stream().mapToInt(personToAge).sum() / people.stream().count();
    ...
    public void task13(){
        ...
        for(String k : keys){
            List<Person> v = people.stream().filter(person -> person.getLastName().equalsIgnoreCase(k)).collect(Collectors.toList());
            personMap.put(k,v);
        }
    ...
    public void task14(){
        ...
        _2020_dates = Stream.iterate(LocalDate.parse("2020-01-01"), date -> date.plusDays(1)).limit(DAYS.between(LocalDate.ofYearDay(2020,1), LocalDate.ofYearDay(2021,1))).toArray(LocalDate[]::new);
